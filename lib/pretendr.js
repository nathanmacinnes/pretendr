...
makePretendr = function (descriptor, mock, asTemplate) {
    ...
    subMakePretendr = function (descriptor, mock, parentMock, propertyName) {
        ...
        // infinite recursion		
		ins.some(function (previousInput, index) {
			if (descriptor === previousInput) {
				out = outs[index];
				return true;
			}
		});

		// if we have, return the corresponding output
		if (out) {
			return out;
		}
...
pretendrFunction = function (attributes, baseObject) {
	...
	// assign this to a variable instead of returning immediately, as we need
	// to reference it inside the function for the instanceof
    fn = function () {
        var call = {},
            instance;
